...
def make_rotation_3d(stepx, stepy):
    ...
    if args.checkpoints == None:
        args.checkpoints = os.path.join(args.dataset, args.prj)
    net = torch.load(os.path.join(logs, args.checkpoints, 'checkpoints',
    ...
    for angle in args.angle_range:
        ...
        for i in range(xp[0].shape[1]):
            ...
            slices[0] = slices[0] - slices[0].min()
            slices[0] = slices[0] / slices[0].max()
        ...
        for j in range(len(all)):
            if angle != 0:
                all[j] = transforms.functional.rotate(all[j], angle=-angle,  #(z, 1, x, y)
                                                      ...
                                                      fill=-1)
            #all[j] = all[j].numpy()[:, 0, ::]  #!!!!!
            all[j] = torch.argmax(all[j], 1).numpy()
...
if __name__ == '__main__':
    ...
    if 0:
        ...
        args.direction = 'xyzori'
        args.destination = '/home/ubuntu/Data/N3D/z4/'
        args.checkpoints = '/media/ExtHDD01/logs/Fly0Bseg/seg0/'
        args.trd = 0
        args.epoch = 400
        args.env = 't09b'
        args.angle_range = list(range(0, 1, 30))
